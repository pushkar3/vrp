...
int			kahng_robins (struct pset *, dist_t, struct edge *);
dist_t			kahng_robins_length (struct pset *, dist_t);
int			rect_mst (struct pset *, struct edge *, bitmap_t *);
dist_t			rect_mst_length (struct pset *);
...
struct edge *	edges;
	...
	nedges = rect_mst (pts, &edges [0], NULL);

	if (nedges NE pts -> n - 1) {
		fatal ("mst_length: Bug 1.");
	}
...
struct point *	p2;
	...
	if (empty_rect EQ NULL) {
		...
		*edges_out = edges;
		...
		nedges = count_empty_rectangles (empty_rect, n);
		nalloc = nedges;
		if (nalloc < at_least) {
			nalloc = at_least;
		}
		...
		*edges_out = edges;
		...
		for (i = 0; i < n; i++, p1++) {
			p2 = (p1 + 1);
			for (j = i + 1; j < n; j++, p2++) {
				if (NOT is_empty_rectangle (empty_rect, i, j)) continue;
...
int		nedges;
	...
	nedges = rect_mst (pts, edges, NULL);
...
struct dsuf	sets;
	...
	sort_edge_list (&edge_array [0], nedges);
	...
	for (i = 0; (i < nterms) AND (n < max_points); i++) {
		...
		for (j = 0; j < nterms; mark_rowp += kmasks, p1++, j++) {
			...
			for (k = 0; k < nterms; p2++, k++) {
				...
				sort_edge_list (&new_edges [0], new_nedges);
