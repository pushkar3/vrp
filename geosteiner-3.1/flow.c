...
void		compute_max_flow (struct flow_prob *,
				  struct flow_temp *,
				  struct flow_soln *);
void		create_flow_solution_data (struct flow_prob *,
					   struct flow_soln *);
void		create_flow_temp_data (struct flow_prob *,
				       struct flow_temp *);
void		free_flow_solution_data (struct flow_soln *);
void		free_flow_temp_data (struct flow_temp *);
	...
	for (;;) {
		...
		*tailp++ = i;
		...
		while (i NE prob -> source) {
			k = pred_arc [i];
			if ((k <= 0) OR (k > num_arcs)) {
				fatal ("compute_max_flow: Bug 1.");
			}
			--k;
			j = arc_src [k];
			if (i EQ j) {
